...
export function createMultiplayerWindow(connect) {
		...
		fetch('http://pb4.eu:9001')
			...
			.then(function (data) {
				...
				array.forEach(function (item) {
					...
					if (item.protocol == gameProtocol || gameProtocol == 0) {
						...
						button.onclick = function () {
							serverInput.value = item.ip;
						};
					} else if (item.protocol < gameProtocol || item.protocol == undefined) {
						cell6.innerHTML = 'Outdated server';
					} else if (item.protocol > gameProtocol) {
