		...
		if (type == 0) {
			...
			if (texture.length == 1 && options.material == undefined) {
				...
				for (let x = 0; x < texture.length; x++) {
					if (
						(texture[x].startsWith('http://') || texture[x].startsWith('https://')) &&
						gameSettings.allowcustom == true
					)
...
function makeCactusMesh(noa, scene, url, name) {
	...
	for (let x = 0; x < 6; x++) {
		...
		if (
			((x < 4 ? url[1] : url[0]).startsWith('http://') || (x < 4 ? url[1] : url[0]).startsWith('https://')) &&
			gameSettings.allowcustom == true
		)
