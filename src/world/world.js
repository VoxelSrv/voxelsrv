...
function WorldGen() {
	...
	function getChunk(id, chunk, x, y, z, state) {
		...
		if (chunkIsStored(id) && result == 'populate') populateChunk(chunk, x, y, z, id)
			...
			var biometable = ndarray(new Array, [24, 24])
	...
	async function generateChunk(chunk, x, y, z, id, biometable) {
		...
		function getHeight(x, y, z, mountaines) {
			var layer1 = heightNoise(x/40, z/40)*20
			var layer2 = heightNoise(x/20, z/20)*20
			var layer3 = heightNoise(x/10, z/10)*20

			var noise = Math.round((layer1*20+layer2*10+layer3*5)/35) + 15
			return noise
		}
	...
	async function populateChunk(chunk, x, y, z, id, biometable) {
			...
			for (var k = 0; k < dz; k++) {
				...
				for (var j = 0; j < dy; j++) {
					if (getBlock(i, j, k) == blockIDs.grass && hash(x+i, z+k) > 0.8 ) {
						...
						if (hash(x+z) >= 0.5) setBlock(i, j+1, k, blockIDs.red_flower)
						else setBlock(i, j+1, k, blockIDs.yellow_flower)
					} else if (getBlock(i, j, k) == blockIDs.grass && hash(x+i*seed, z+k*seed) < 0.01 && biome == 'plants' ) {
						tree(i, j, k, y+j)
					} else if (getBlock(i, j, k) == blockIDs.grass && hash(x+i*seed, z+k*seed) < 0.8 && biome == 'forest' ) {
						tree(i, j, k, y+j)
		...
		async function setBlock(x, y, z, block) {
			if (x >= 0 && x < dx && y >= 0 && y < dy && z >= 0 && z < dz) { chunk.set(x, y, z, block) }
				...
				var tempChunk = new ndarray( new Uint16Array(24 * 24 * 24), [24, 24, 24] )
