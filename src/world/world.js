...
function WorldGen() {
	...
	this.onMessage = function (ev){
		...
		if (msg=='init') {
			...
			caveNoise = makeNoise3D(seed+32454)
			caveNoise2 = makeNoise3D(seed+70323)
			biomeNoise1 = makeNoise2D(seed+685746563)
			biomeNoise2 = makeNoise2D(seed+576)
			biomeNoise3 = makeNoise2D(seed+8978876)
	...
	var biomeSpacing = 250
	function generateChunk(chunk, x, y, z, id, biometable) {
		...
		for (var i = 0; i < dx; i++) {
			for (var k = 0; k < dz; k++) {
				if (Math.abs(id.split("|")[1]) == 24) {
					...
					var temperature = Math.floor(Math.abs(biomeNoise1((x + i)/(biomeSpacing), (z + k)/(biomeSpacing)))*12)
					var mountaines = Math.abs(biomeNoise2((x + i)/200, (z + k)/200))
					var biomerng = Math.floor(Math.abs(biomeNoise3((x + i)/biomeSpacing, (z + k)/biomeSpacing))*12)
		...
		function getBlock(x, y, z, height, biome) {
			if (y <= 0 && y <= height) return blockIDs.stone
		...
		function getBiome(x, z, temperature, mountaines, biomerng) {
			var rnd = hash(x, z)/2
			if (3.5 < temperature + rnd && biomerng + rnd > 5) return 'desert'
			else if ( 2.5 < temperature + rnd < 5) return 'plants'
			else if ( 2.5 < temperature + rnd < 5) return 'forest'
			else if ( 0 < temperature + rnd < 3 && biomerng + rnd < 4) return 'iceland'
	...
	function populateChunk(chunk, x, y, z, id, biometable) {
		var toUpdate = {}
		var dx = chunk.shape[0]
		var dy = chunk.shape[1]
		var dz = chunk.shape[2]
			...
			for (var k = 0; k < dz; k++) {
				...
				for (var j = 0; j < dy; j++) {
					if (getBlock(i, j, k) == blockIDs.stone) {
						if (y < 30 && hash(x+i, y+j, z+k) <= 0.05) setBlock(i, j, k, blockIDs.coal_ore)
						if (y < 15 && hash(x+i*2, y+j*2, z+k*2) <= 0.01) setBlock(i, j, k, blockIDs.iron_ore) 

					} else if (getBlock(i, j+1, k) == 0 && getBlock(i, j, k) == blockIDs.grass && hash(x+i, z+k) > 0.8 ) {
						setBlock(i, j+1, k, blockIDs.grass_plant)
					} else if (getBlock(i, j+1, k) == 0 && getBlock(i, j, k) == blockIDs.sand && hash(x+i, z+k) < 0.02 ) {
						setBlock(i, j+1, k, blockIDs.deadbush)
					} else if (getBlock(i, j, k) == blockIDs.grass && hash((x+i)*20, (z+k)*20) < 0.05 ) {
